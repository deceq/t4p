...
class Application(object):
		...
		for url, method in urls:
                        handler = type(method.__name__+'Handler', (Handler,), dict(handler=tornado.gen.coroutine(method)) )
                        urle.append( (url, handler) )
                print(urle)
                self.app = tornado.web.Application(urle, cookie_secret=config['cookie_secret'])
	...
	def load_templates(self, path=None):
		...
		for item in templates:
			...
			if os.path.isfile(_item):
				...
				key = 'template:{template_path}'.format(template_path=template_name)
	                        value = {
	                                "content": "",
