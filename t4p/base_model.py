...
class BaseModel(object):
	...
	def generate_ajax_form(cls):
		...
		form = '''	<div id="{div_id}"
			<form action="#">
			{fields}
			<button type="submit" class="btn btn-default" onClick="{method_name}()">Submit</button>
			</form>'''
		...
		for item in dir(cls):
			if item.startswith('f_'):
				...
				fields += field.format(field_name=field_name, upper_field_name=field_name.upper())
		...
		js_method = js_method % (method_name, js_fields, post_fields, div_id, div_id, div_id)
		form = form.format(fields=fields, method_name=method_name, div_id=div_id, js_fields=js_fields, post_fields=post_fields)
